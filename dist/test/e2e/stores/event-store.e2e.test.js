...
let eventStore;
beforeAll(async () => {
    db = await (0, database_init_1.default)('event_store_serial', no_logger_1.default);
    stores = db.stores;
...
test('Should include id and createdAt when saving', async () => {
    ...
    const seen = [];
    eventStore.on(events_1.APPLICATION_CREATED, (e) => seen.push(e));
...
test('Should include empty tags array for new event', async () => {
    ...
    const promise = new Promise((resolve) => {
        eventStore.on(events_1.FEATURE_CREATED, (storedEvent) => {
...
test('Should get all events of type', async () => {
    ...
    await Promise.all([0, 1, 2, 3, 4, 5].map(async (id) => {
        const event = id % 2 == 0
            ? new events_1.FeatureCreatedEvent({
            ...
            : new events_1.FeatureDeletedEvent({
                project: data.project,
                preData: data,
                featureName: data.name,
                createdBy: 'test-user',
