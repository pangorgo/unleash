...
let db;
beforeAll(async () => {
    db = await (0, database_init_1.default)('user_store_serial', no_logger_1.default);
    stores = db.stores;
});
afterAll(async () => {
    await db.destroy();
...
test('should not get password_hash for unknown userId', async () => {
    const store = stores.userStore;
    await expect(async () => store.getPasswordHash(-12)).rejects.toThrow(new notfound_error_1.default('User not found'));
});
...
test('should delete user', async () => {
    ...
    await expect(() => stores.userStore.get(user.id)).rejects.toThrow(new notfound_error_1.default('No user found'));
});
