...
const createProjects = async (projects = [types_1.DEFAULT_PROJECT]) => {
...
const createFeatureToggle = async (feature, strategies = [mockStrategy()], project = types_1.DEFAULT_PROJECT, environment = util_1.DEFAULT_ENV, expectStatusCode = 201, expectSegmentStatusCodes = [
    { status: 200 },
]) => {
...
const updateFeatureStrategy = async (featureName, strategy, project = types_1.DEFAULT_PROJECT, environment = util_1.DEFAULT_ENV, expectedStatus = 200) => {
...
beforeAll(async () => {
    ...
    app = await (0, test_helper_1.setupAppWithCustomConfig)(db.stores, {}, db.rawDatabase);
});
afterAll(async () => {
    await app.destroy();
    await db.destroy();
...
test('should validate feature strategy segment limit', async () => {
    ...
    await createFeatureToggle(mockFeatureToggle(), [mockStrategy(segments.map((s) => s.id))], types_1.DEFAULT_PROJECT, util_1.DEFAULT_ENV, 201, [
        {
            status: 400,
            message: `Strategies may not have more than ${segments_1.DEFAULT_STRATEGY_SEGMENTS_LIMIT} segments`,
        },
    ]);
});
...
describe('project-specific segments', () => {
    test(`can create a toggle with a project-specific segment`, async () => {
        ...
        const project = (0, random_id_1.randomId)();
    ...
    test(`can't create a toggle with a segment from a different project`, async () => {
        const segmentName = 'my-segment';
        const project1 = (0, random_id_1.randomId)();
        const project2 = (0, random_id_1.randomId)();
        ...
        await createFeatureToggle({
            name: 'first_feature',
            description: 'the #1 feature',
        }, [strategy], project2, util_1.DEFAULT_ENV, 201, [{ status: 400 }]);
    });
    ...
    test(`can't set a specific segment project when being used by multiple projects (global)`, async () => {
        ...
        await expect(() => updateSegment(segment.id, {
            ...segment,
            project: project1,
        })).rejects.toThrow(`Invalid project. Segment is being used by strategies in other projects: ${project1}, ${project2}`);
    });
});
