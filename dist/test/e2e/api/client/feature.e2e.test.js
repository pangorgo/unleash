...
let db;
beforeAll(async () => {
    db = await (0, database_init_1.default)('feature_api_client', no_logger_1.default);
    ...
    await app.services.featureToggleServiceV2.saveVariants('feature.with.variants', 'default', [
        {
            name: 'control',
            weight: 50,
            weightType: 'fix',
...
test('Can get strategies for specific environment', async () => {
    ...
    const env = constants_1.DEFAULT_ENV;
    ...
    await app.services.environmentService.addEnvironmentToProject('testing', 'default');
    await app.request
    ...
    await app.request
        ...
        expect(res.body.strategies.find((s) => s.name === 'custom1')).toBeDefined();
    });
});
...
test('returns a feature toggles impression data for a different project', async () => {
    ...
    return app.request
        ...
        .expect('Content-Type', /json/)
        .expect((res) => {
        const projectToggle = res.body.features.find((resToggle) => resToggle.project === project.id);
