...
const api_user_1 = __importDefault(require("../../../../../lib/types/api-user"));
...
const createStrategy = async (featureName, payload) => {
    return app.request
        .post(`/api/admin/projects/default/features/${featureName}/environments/default/strategies`)
        .send(payload)
        .expect(200);
};
const updateStrategy = async (featureName, strategyId, payload) => {
...
test('Trying to toggle environment that does not exist yields 404', async () => {
    await app.request
        ...
        .expect(201);
    await app.request
        .post('/api/admin/projects/default/features/toggle.env/environments/does-not-exist/on')
        .send({})
        .expect(404);
    await app.request
        .post('/api/admin/projects/default/features/toggle.env/environments/does-not-exist/off')
        .send({})
        .expect(404);
});
...
test('Should not allow changing project to target project without the same enabled environments', async () => {
    ...
    //@ts-ignore
    user, 'default')).rejects.toThrow(new incompatible_project_error_1.default(targetProject));
});
...
test('should handle strategy variants', async () => {
    const feature = { name: (0, uuid_1.v4)(), impressionData: false };
...
test('should allow long parameter values', async () => {
    ...
    await app.request
        .post(`/api/admin/projects/${project.id}/features/${toggle.name}/environments/default/strategies`)
        .send(strategy)
        .expect(200);
});
test('should change strategy sort order when payload is valid', async () => {
    const toggle = { name: (0, uuid_1.v4)(), impressionData: false };
    await app.request
        .post('/api/admin/projects/default/features')
        .send({
        name: toggle.name,
    ...
    const { body: strategyOne } = await app.request
        .post(`/api/admin/projects/default/features/${toggle.name}/environments/default/strategies`)
        .send({
        name: 'default',
        parameters: {
            userId: 'string',
    ...
    const { body: strategyTwo } = await app.request
        ...
        .expect(200);
    const { body: strategies } = await app.request.get(`/api/admin/projects/default/features/${toggle.name}/environments/default/strategies`);
    expect(strategies[0].sortOrder).toBe(0);
    expect(strategies[0].id).toBe(strategyOne.id);
    expect(strategies[1].sortOrder).toBe(1);
    expect(strategies[1].id).toBe(strategyTwo.id);
    await app.request
        .post(`/api/admin/projects/default/features/${toggle.name}/environments/default/strategies/set-sort-order`)
        .send([
        {
            id: strategyOne.id,
            sortOrder: 2,
        ...
        .expect(200);
    const { body: strategiesOrdered } = await app.request.get(`/api/admin/projects/default/features/${toggle.name}/environments/default/strategies`);
...
test('should return strategies in correct order when new strategies are added', async () => {
    ...
    await app.request
        ...
        .expect(200);
    const { body: strategiesReOrdered } = await app.request.get(`/api/admin/projects/default/features/${toggle.name}/environments/default/strategies`);
...
test('Can query for features with namePrefix and tags', async () => {
    ...
    await db.stores.featureTagStore.tagFeature('to-be-tagged-nameprefix-and-tags', tag);
    await db.stores.featureTagStore.tagFeature('tagged-but-not-hit-nameprefix-and-tags', tag);
    await app.request
...
test('Can query for two tags at the same time. Tags are ORed together', async () => {
    ...
    await db.stores.featureTagStore.tagFeature(taggedWithSecond.name, secondTag);
...
test('Should batch stale features', async () => {
    ...
    await app.request
        .post(`/api/admin/projects/${types_1.DEFAULT_PROJECT}/stale`)
        .send({
        ...
        .expect(202);
    const { body } = await app.request
        .get(`/api/admin/projects/${types_1.DEFAULT_PROJECT}/features/${staledFeatureName1}`)
        .expect(200);
...
test('should disable strategies in place', async () => {
    ...
    const { body: updatedStrategyOne } = await app.request
        .put(`/api/admin/projects/default/features/${toggle.name}/environments/default/strategies/${strategyOne.id}`)
        .send({
        name: 'flexibleRollout',
        constraints: [],
        disabled: true,
        ...
        .expect(200);
    const { body: updatedStrategies } = await app.request.get(`/api/admin/projects/default/features/${toggle.name}/environments/default/strategies`);
...
//# sourceMappingURL=features.e2e.test.js.map