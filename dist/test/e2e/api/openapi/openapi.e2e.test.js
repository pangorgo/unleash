...
const openapi_tags_1 = require("../../../../lib/openapi/util/openapi-tags");
let app;
let db;
beforeAll(async () => {
    db = await (0, database_init_1.default)('openapi', no_logger_1.default);
...
test('should serve the OpenAPI spec with a `version` property', async () => {
    return app.request
        ...
        expect(semver_1.default.clean(version)).toStrictEqual(version);
        // ensure the version listed is valid semver
        expect(semver_1.default.parse(version, { loose: false })).toBeTruthy();
    });
});
test('the generated OpenAPI spec is valid', async () => {
    ...
    try {
        await swagger_parser_1.default.validate(body);
    }
    catch (err) {
        console.error(err);
    ...
    const [, enforcerError, enforcerWarning] = await (0, openapi_enforcer_1.default)(body, {
        fullResult: true,
        componentOptions: {
            exceptionSkipCodes: [
...
test('all root-level tags are "approved tags"', async () => {
    ...
    const approvedTags = openapi_tags_1.openApiTags;
