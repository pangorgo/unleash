...
const permissions_1 = require("../../../lib/types/permissions");
...
beforeAll(async () => {
    db = await (0, database_init_1.default)('project_service_serial', no_logger_1.default);
    stores = db.stores;
    ...
    environmentService = new environment_service_1.default(stores, config);
...
test('should not be able to delete project with toggles', async () => {
    ...
    try {
        ...
        expect(err.message).toBe('You can not delete a project with active feature toggles');
    }
});
...
test('should get list of users with access to project', async () => {
    ...
    const owner = await stores.roleStore.getRoleByName(model_1.RoleName.OWNER);
...
test('should add a member user to the project', async () => {
    ...
    await projectService.addUser(project.id, memberRole.id, projectMember2.id, 'test');
    const { users } = await projectService.getAccessToProject(project.id);
...
test('should add admin users to the project', async () => {
    ...
    await projectService.addUser(project.id, ownerRole.id, projectAdmin1.id, 'test');
    await projectService.addUser(project.id, ownerRole.id, projectAdmin2.id, 'test');
    const { users } = await projectService.getAccessToProject(project.id);
...
test('add user should fail if user already have access', async () => {
    ...
    const memberRole = await stores.roleStore.getRoleByName(model_1.RoleName.MEMBER);
    await projectService.addUser(project.id, memberRole.id, projectMember1.id, 'test');
    await expect(async () => projectService.addUser(project.id, memberRole.id, projectMember1.id, 'test')).rejects.toThrow(new Error('User already has access to project=add-users-twice'));
});
...
test('should not change project if feature toggle project does not match current project id', async () => {
    ...
    try {
        ...
        expect(err.message.includes(permissions_1.MOVE_FEATURE_TOGGLE)).toBeTruthy();
    }
});
...
test('should require equal project environments to move features', async () => {
    ...
    const environment = { name: (0, random_id_1.randomId)(), type: 'production' };
    const toggle = { name: (0, random_id_1.randomId)() };
    await projectService.createProject(projectA, user);
    await projectService.createProject(projectB, user);
    await featureToggleService.createFeatureToggle(projectA.id, toggle, user);
...
test('should change a users role in the project', async () => {
    ...
    let { users: updatedUsers } = await projectService.getAccessToProject(project.id);
...
test('Should allow bulk update of group permissions', async () => {
    ...
    await projectService.addAccess(project.id, createFeatureRole.id, {
        users: [{ id: user1.id }],
        groups: [{ id: group1.id }],
    }, 'some-admin-user');
});
test('Should bulk update of only users', async () => {
    ...
    await projectService.addAccess(project, createFeatureRole.id, {
        users: [{ id: user1.id }],
        groups: [],
    }, 'some-admin-user');
});
...
test('should calculate average time to production', async () => {
    ...
    const featureToggles = await Promise.all(toggles.map((toggle) => {
        return featureToggleService.createFeatureToggle(project.id, toggle, user);
    }));
    await Promise.all(featureToggles.map((toggle) => {
        return stores.eventStore.store(new events_1.FeatureEnvironmentEvent({
            enabled: true,
            project: project.id,
            featureName: toggle.name,
            environment: 'default',
    ...
    await Promise.all(featureToggles.map((toggle) => updateFeature(toggle.name, { created_at: (0, date_fns_1.subDays)(new Date(), 15) })));
    await updateFeature('average-prod-time-5', {
        created_at: (0, date_fns_1.subDays)(new Date(), 33),
    });
...
test('should calculate average time to production ignoring some items', async () => {
    ...
    await stores.eventStore.store(new events_1.FeatureEnvironmentEvent(makeEvent(toggle.name)));
    // ignore events added after first enabled
    await updateEventCreatedAt((0, date_fns_1.addDays)(new Date(), 1), toggle.name);
    await stores.eventStore.store(new events_1.FeatureEnvironmentEvent(makeEvent(toggle.name)));
    ...
    await stores.eventStore.store(new events_1.FeatureEnvironmentEvent({
        ...makeEvent(devToggle.name),
        environment: 'customEnv',
    }));
    // ignore toggles from other projects
    const otherProjectToggle = { name: 'other-project' };
    await featureToggleService.createFeatureToggle('default', otherProjectToggle, user);
    await stores.eventStore.store(new events_1.FeatureEnvironmentEvent(makeEvent(otherProjectToggle.name)));
    // ignore non-release toggles
    const nonReleaseToggle = { name: 'permission-toggle', type: 'permission' };
    await featureToggleService.createFeatureToggle(project.id, nonReleaseToggle, user);
    await stores.eventStore.store(new events_1.FeatureEnvironmentEvent(makeEvent(nonReleaseToggle.name)));
    // ignore toggles with events before toggle creation time
    const previouslyDeleteToggle = { name: 'previously-deleted' };
    await featureToggleService.createFeatureToggle(project.id, previouslyDeleteToggle, user);
    await stores.eventStore.store(new events_1.FeatureEnvironmentEvent(makeEvent(previouslyDeleteToggle.name)));
...
test('should get correct amount of features created in current and past window', async () => {
    ...
    await Promise.all([
        ...
        updateFeature(toggles[3].name, { created_at: (0, date_fns_1.subDays)(new Date(), 31) }),
    ]);
    const result = await projectService.getStatusUpdates(project.id);
...
test('should get correct amount of features archived in current and past window', async () => {
    ...
    await Promise.all([
        ...
        updateFeature(toggles[2].name, {
            archived_at: (0, date_fns_1.subDays)(new Date(), 31),
            archived: true,
        }),
...
test('should get correct amount of project members for current and past window', async () => {
    ...
    const memberRole = await stores.roleStore.getRoleByName(model_1.RoleName.MEMBER);
    await Promise.all(createdUsers.map((createdUser) => projectService.addUser(project.id, memberRole.id, createdUser.id, 'test')));
    const result = await projectService.getStatusUpdates(project.id);
