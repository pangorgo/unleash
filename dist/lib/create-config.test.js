...
test('should add initApiToken for admin token from options', async () => {
    ...
    expect(config.authentication.initApiTokens[0].type).toBe(api_token_1.ApiTokenType.ADMIN);
});
test('should add initApiToken for client token from options', async () => {
    ...
    expect(config.authentication.initApiTokens[0].environment).toBe(token.environment);
    expect(config.authentication.initApiTokens[0].project).toBe(token.project);
    expect(config.authentication.initApiTokens[0].type).toBe(api_token_1.ApiTokenType.CLIENT);
});
...
test('should validate initApiToken for admin token from env var', async () => {
    ...
    expect(() => (0, create_config_1.createConfig)({})).toThrow('Admin token cannot be scoped to single project');
    delete process.env.INIT_ADMIN_API_TOKENS;
});
test('should validate initApiToken for client token from env var', async () => {
    process.env.INIT_CLIENT_API_TOKENS = '*:*:some-token1';
    expect(() => (0, create_config_1.createConfig)({})).toThrow('Client token cannot be scoped to all environments');
...
test('Environment variables for frontend CORS origins takes priority over options', async () => {
    const create = (frontendApiOrigins) => {
        return (0, create_config_1.createConfig)({
            frontendApiOrigins,
        }).frontendApiOrigins;
    };
...
test('baseUriPath defaults to the empty string', async () => {
    let config = (0, create_config_1.createConfig)({});
